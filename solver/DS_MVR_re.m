...
for it = 1:max_it
    ...
    if mod(it,out_iter)==0
        ...
        disp(['CPD-DS-M1-VR at iteration ',num2str(mm+1),' and the MSE is ',num2str(MSE_A(mm+1))])
        disp(['CPD-DS-M1-VR at iteration ',num2str(mm+1),' and the NRE is ',num2str(NRE_A(mm+1))])
        disp('====')
        mm = mm + 1;
        a=tic;
    end
